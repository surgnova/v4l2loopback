...
struct rgb_setup process_args(int argc, char **argv)
{
	prog = argv[0];
	struct rgb_setup setup;
	if (argc != 6){
		...
		setup.frame_bytes = 3 * setup.frame_height * setup.frame_width;
		setup.fps = atof(argv[5]);
	}
	return setup;
}

void copy_frames(struct rgb_setup setup, int dev_fd)
{
/*
...
*/
    ...
    memset(frame, 0, setup.frame_bytes);
	...
	while (1) {
        ...
        if (!(loop_count % 150)) {
            int i, j;
            for (i = 0; i < setup.frame_height; i++)
                for (j = 0; j < setup.frame_width; j++) {
                    int index = i * setup.frame_width * 3 + j*3;
                    frame[index] = rand() % 255;
                    frame[index + 1] =  rand() % 255;
                    frame[index + 2] =  rand() % 255;
            }
        }
...
int
open_video(struct rgb_setup setup)
{
...
{
	struct rgb_setup loc_setup = process_args(argc, argv);
